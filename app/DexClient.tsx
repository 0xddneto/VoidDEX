...
export default function VoidDex({initialStates}:{initialStates:DexPoolState[]}){
 ...
 return <main className="shell">
  <header className="top"><a className="brand" href="/"><i/>VOID<b>DEX</b></a><nav className="nav"><a className="active" href="#swap">Swap</a><a href="#pool">Pool</a><a href="https://voidscan.voidshub.com">Explore</a></nav><button className="wallet" onClick={connect}>{account?`${account.slice(0,6)}…${account.slice(-4)}`:'Connect wallet'}</button></header>
  ...
  <footer className="foot"><span>DEX app {DEX_APP.slice(0, 6)}…{DEX_APP.slice(-4)}</span><span>Runtime {RUNTIME}</span><a href="https://voidscan.voidshub.com">Open VoidScan →</a></footer>
 </main>;
}
